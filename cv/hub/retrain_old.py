...
def save_graph_to_file(graph, graph_file_name, module_spec, class_count):
  ...
  tf.gfile.MakeDirs(os.path.dirname(graph_file_name))
...
def main(_):
  ...
  with tf.Session(graph=graph) as sess:
    ...
    for i in range(FLAGS.how_many_training_steps):
      ...
      # Log train summary every so often
      is_last_step = (i + 1 == FLAGS.how_many_training_steps)
      if (i % FLAGS.train_summary_interval) == 0 or is_last_step:
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=int,
      default=10,
      help='How often to log train summary.'
  )
  parser.add_argument(
      '--eval_step_interval',
      type=int,
